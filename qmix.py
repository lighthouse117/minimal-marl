...
class ReplayBuffer:
    ...
    def sample_chunk(self, batch_size, chunk_size):
        start_idx = np.random.randint(
            0, len(self.buffer) - chunk_size, batch_size)
...
def train(q, q_target, mix_net, mix_net_target, memory, optimizer, gamma, batch_size, update_iter=10, chunk_size=10,
    ...
    for _ in range(update_iter):
        ...
        mix_net_hidden = [torch.empty_like(
            mix_net_target_hidden) for _ in range(_chunk_size + 1)]
        ...
        for step_i in range(_chunk_size):
            ...
            q_a = q_out.gather(
                2, a[:, step_i, :].unsqueeze(-1).long()).squeeze(-1)
            ...
            target_q = r[:, step_i, :].sum(
                dim=1, keepdims=True) + (gamma * q_prime_total * (1 - done[:, step_i]))
            ...
            mix_net_hidden[step_i +
                           1][~done_mask] = next_mix_net_hidden[~done_mask]
        ...
        torch.nn.utils.clip_grad_norm_(
            q.parameters(), grad_clip_norm, norm_type=2)
        torch.nn.utils.clip_grad_norm_(
...
def main(env_name, lr, gamma, batch_size, buffer_limit, log_interval, max_episodes,
    ...
    test_env = Monitor(env, directory='recordings',
    ...
    for episode_i in range(max_episodes):
        ...
        with torch.no_grad():
            ...
            while not all(done):
                ...
                memory.put((state, action, (np.array(reward)).tolist(),
                            next_state, [int(all(done))]))
                score += sum(reward)
                state = next_state

        if memory.size() > warm_up_steps:
            train(q, q_target, mix_net, mix_net_target, memory,
                  optimizer, gamma, batch_size, update_iter, chunk_size)
...
if __name__ == '__main__':
    ...
    if USE_WANDB:
        ...
        run = wandb.init(project='minimal-marl',
